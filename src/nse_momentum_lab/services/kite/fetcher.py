...
class KiteFetcher:
    def __init__(self, auth: KiteAuth | None = None) -> None:
        ...
        while True:
            ...
            try:
                ...
                backoff = min(
                    ...
                    + random.random() * RETRY_JITTER_SECONDS,
                )
                logger.warning(
    ...
    def _normalize_5min_candles(
        ...
        frame["date"] = pd.to_datetime(frame["date"]).dt.date
        frame["candle_time"] = pd.to_datetime(frame["candle_time"], utc=True).dt.tz_convert(IST)
        frame["volume"] = frame["volume"].astype("int64")
        return frame
