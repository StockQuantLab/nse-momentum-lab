...
class TestJobRunOperations:
    async def test_insert_job(self, db_session, clean_db):
        ...
        assert job.job_run_id is not None
    ...
    async def test_update_job_status(self, db_session, clean_db):
        ...
        result = await db_session.execute(select(JobRun).where(JobRun.job_run_id == job.job_run_id))
    ...
    async def test_commit_with_session_begin(self, db_sessionmaker):
        ...
        async with db_sessionmaker() as session:
            async with session.begin():
                symbol = RefSymbol(symbol="COMMITTEST", series="EQ", status="ACTIVE")
                session.add(symbol)

        async with db_sessionmaker() as session:
            result = await session.execute(
                select(RefSymbol).where(RefSymbol.symbol == "COMMITTEST")
            )
